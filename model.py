...
class Model(pl.LightningModule):
    ...
    def __init__(self, hparams, meta_data_vocab=None):
        ...
        if hparams.iterative_layers != 0:
            self._iterative_transformer = \
                self._base_model.encoder.layer[-hparams.iterative_layers:]
            self._base_model.encoder.layer = \
                self._base_model.encoder.layer[:-hparams.iterative_layers]
        else:
            self._iterative_transformer = []
        ...
        self._labelling_layer = nn.Linear(self._labelling_dim,
                                          self._num_labels)
        ...
        self._metric = Carb(
            hparams) if hparams.task == 'oie' else Conjunction()
    ...
    def configure_optimizers(self):
        ...
        params = [{"params": [p for n, p in all_params if not any(
            nd in n for nd in no_decay) and 'base_model' in n],
                   "weight_decay_rate": 0.01, 'lr': self.hparams.lr},
                  {"params": [p for n, p in all_params if any(
                      nd in n for nd in no_decay) and 'base_model' in n],
                   "weight_decay_rate": 0.0, 'lr': self.hparams.lr},
                  {"params": [p for n, p in all_params if
                              'base_model' not in n], 'lr': self.hparams.lr}]
    ...
    def forward(self, batch, mode='train', batch_idx=-1, constraints=None,
                cweights=None):
        ...
        while True:
            ...
            word_hidden_states = \
                torch.gather(hidden_states,
                             1,
                             batch.word_starts.unsqueeze(2). \
        ...
        for d, word_scores in enumerate(all_depth_scores):
            if mode == 'train':
                ...
                mask = torch.ones(batch.word_starts.shape).int().type_as(
                    hidden_states)
                loss += self._loss(
                    word_scores.reshape(batch_size * num_words, -1),
                    batch.labels[:, d, :].reshape(-1))
            else:
        ...
        if mode == 'train':
            if constraints != '':
                all_depth_scores = torch.cat(
                    [d.unsqueeze(1) for d in all_depth_scores], dim=1)
                all_depth_scores = torch.softmax(all_depth_scores, dim=-1)

                const_loss = self.constrained_loss(all_depth_scores, batch,
                                                   constraints,
                                                   cweights) / batch_size
                loss = const_loss
            ...
            if self.hparams.wreg != 0:
                ...
                for name in self._initial_parameters:
                    weight_diff += torch.norm(
                        current_parameters[name] - self._initial_parameters[
                            name])
            ...
            if constraints != '' and \
                    'predict' not in self.hparams.mode and \
    ...
    def constrained_loss(self, all_depth_scores, batch, constraints, cweights):
        ...
        verb_scores = torch.gather(all_depth_scores, 2,
                                   batch.verb_index.unsqueeze(1).unsqueeze(
                                       3).repeat(1, depth, 1, labels))
        verb_rel_scores = verb_scores[:, :, :, 2]
        # (batch_size, depth, num_words)
        verb_rel_scores = verb_rel_scores * (batch.verb_index != 0).unsqueeze(
            1).float()
        ...
        if 'hvr' in constraints:
            row_rel_loss = F.relu(batch.verb.sum(dim=1).float() -
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=-1):
        ...
        output_dict = self.forward(batch, mode='train', batch_idx=batch_idx,
                                   constraints=constraints, cweights=cweights)
        ...
        if self.hparams.mode != 'test':
            if self.hparams.write_async:
                t = Thread(target=self.write_to_file,
                           args=(output, batch_idx, self.hparams.task))
                t.start()
            else:
            ...
            if 'predict' in self.hparams.mode:
                ...
                for output in outputs:
                    if type(output['meta_data'][0]) != type(""):
                        output['meta_data'] = [self._meta_data_vocab.itos[m]
                                               for m in output['meta_data']]
                    self._metric(output['predictions'], output['ground_truth'],
    ...
    def write_to_file(self, output, batch_idx, task):
        ...
        output['meta_data'] = [self._meta_data_vocab.itos[m] for m in
                               output['meta_data']]
        if task == 'oie':
            ...
            for example_id, sentence in enumerate(all_predictions):
                ...
                for extraction in predicted_extractions:
                    ...
                    ext_str = f'{sentence}\t<arg1> ' \
                              f'{extraction.args[0]} </arg1> <rel> ' \
                              f'{extraction.pred} </rel> <arg2> ' \
                              f'{args1} </arg2>\t{extraction.confidence}\n'
        ...
        if task == 'oie' and self.hparams.write_allennlp:
            if batch_idx == 0:
                predictions_f_allennlp = open(f'{self.hparams.out}.allennlp',
                                              'w')
