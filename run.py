...
def train(hparams, checkpoint_callback, meta_data_vocab, train_dataloader,
    ...
    trainer = Trainer(show_progress_bar=True,
                      ...
                      gpus=hparams.gpus, logger=logger,
                      checkpoint_callback=checkpoint_callback,
                      min_epochs=hparams.epochs,
                      max_epochs=hparams.epochs,
                      accumulate_grad_batches=
                        int(hparams.accumulate_grad_batches),
                      gradient_clip_val=hparams.gradient_clip_val,
                      num_tpu_cores=hparams.num_tpu_cores,
                      use_tpu=hparams.use_tpu,
                      train_percent_check=hparams.train_percent_check,
                      track_grad_norm=hparams.track_grad_norm)
    # val_percent_check=0, max_steps=hparams.max_steps,
...
def resume(hparams, checkpoint_callback, meta_data_vocab, train_dataloader,
    ...
    loaded_hparams_dict = datpro.override_args(
        loaded_hparams_dict, current_hparams_dict, sys.argv[1:])
    loaded_hparams = datpro.convert_to_namespace(loaded_hparams_dict)
...
# We can probably merge predict and test. and removing caching
# when only testing - VA
def test(hparams, checkpoint_callback, meta_data_vocab,
         train_dataloader, val_dataloader, test_dataloader,
         all_sentences, mapping=None, conj_word_mapping=None):
    checkpoint_paths = get_checkpoint_path(hparams)
    if not 'train' in hparams.mode:
        if has_cuda:
            loaded_hparams_dict = torch.load(checkpoint_paths[0])['hparams']
        else:
            loaded_hparams_dict = torch.load(checkpoint_paths[0],
                                map_location=torch.device('cpu'))['hparams']
        current_hparams_dict = vars(hparams)
        loaded_hparams_dict = datpro.override_args(loaded_hparams_dict,
                                    current_hparams_dict, sys.argv[1:])
        loaded_hparams = datpro.convert_to_namespace(loaded_hparams_dict)
    else:
...
def predict(hparams, checkpoint_callback, meta_data_vocab,
    ...
    logger = None
    trainer = Trainer(gpus=hparams.gpus, logger=logger,
                      resume_from_checkpoint=checkpoint_path)
...
def splitpredict(hparams, checkpoint_callback,
                 meta_data_vocab, train_dataloader,
                 val_dataloader, test_dataloader, all_sentences):
    ...
    if hparams.split_fp == '':
        ...
        model = predict(hparams, None, meta_data_vocab,
                        None, None, test_dataloader, all_sentences)
    ...
    _, _, split_test_dataset, meta_data_vocab, _ = \
        datpro.make_datasets(hparams, sentences)
    split_test_dataloader = DataLoader(split_test_dataset,
                                       batch_size=hparams.batch_size,
                                       collate_fn=datpro.pad_data,
                                       num_workers=1)
    ...
    if hparams.rescoring:    
        ...
        for sentence_str in model.all_predictions_oie:
            ...
            sentence_line_nums.add(curr_line_num)
...
def get_labels(hparams, model, sentences, orig_sentences, sentences_indices):
    label_dict = {0 : 'NONE', 1 : 'ARG1', 2 : 'REL',
                  3 : 'ARG2', 4 : 'ARG2', 5 : 'NONE'}
    lines = []
    ...
    for i in range(0, len(sentences_indices)):
        ...
        for j in range(0, len(sentences_indices[i])):
            assert len(sentences_indices[i][j]) == \
                   len(outputs[idx1]['meta_data'][idx2].strip().split()), \
                ipdb.set_trace()
            sentence = outputs[idx1]['meta_data'][idx2].strip() +\
                       ' [unused1] [unused2] [unused3]'
...
def prepare_test_dataset(hparams, model, sentences, orig_sentences,
    ...
    for i in range(0, len(sentences_indices)):
        ...
        for j in range(0, len(sentences_indices[i])):
            try:
                assert len(sentences_indices[i][j]) == len(outputs[idx1][
                        'meta_data'][idx2].strip().split()), ipdb.set_trace()
            except:
                ipdb.set_trace()
            sentence = outputs[idx1]['meta_data'][idx2].strip() + \
                       ' [unused1] [unused2] [unused3]'
            ...
            if hparams.rescoring == 'others':
                for ext_i, extraction in enumerate(all_extractions):
                    other_extractions = ' '.join(all_extractions[:ext_i]+
                                                 all_extractions[ext_i+1:])
                    other_len_exts = sum(len_exts[:ext_i])+\
                                     sum(len_exts[ext_i+1:])
                    input = original_sentence + ' ' + other_extractions
                    lines.append(input)
                    output = all_str_labels[ext_i] + ' ' + \
                             ' '.join(['NONE'] * other_len_exts)
                    lines.append(output)
...
def run(hparams):
    ...
    if hparams.task == 'conj':
        hparams.train_fp = 'data/ptb-train.labels' if \
            hparams.train_fp == None else hparams.train_fp
        hparams.dev_fp = 'data/ptb-dev.labels' if \
            hparams.dev_fp == None else hparams.dev_fp
        hparams.test_fp = 'data/ptb-test.labels' if\
            hparams.test_fp == None else hparams.test_fp
        if hparams.debug:
            hparams.train_fp = hparams.dev_fp = hparams.test_fp\
                = 'data/debug_conj.labels'
    elif hparams.task == 'oie':
        hparams.train_fp = 'data/openie4_labels' if \
            hparams.train_fp == None else hparams.train_fp
        hparams.dev_fp = 'carb/data/dev.txt' if \
            hparams.dev_fp == None else hparams.dev_fp
        hparams.test_fp = 'carb/data/test.txt' if \
            hparams.test_fp == None else hparams.test_fp
        if hparams.debug:
            hparams.train_fp = hparams.dev_fp = hparams.test_fp =\
                'data/debug_oie.labels'

    hparams.gradient_clip_val = 5 if hparams.gradient_clip_val == None \
        else float(hparams.gradient_clip_val)

    train_dataset, val_dataset, test_dataset, meta_data_vocab, all_sentences =\
        datpro.make_datasets(hparams)
