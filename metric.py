from carb.benchmark import Benchmark
...
def get_coords(all_depth_labels, tokens=None, correct=False):
    ...
    for depth in range(len(all_depth_labels)):
        ...
        coordphrase, conjunction, coordinator, separator =\
        ...
        for i, label in enumerate(labels):
            ...
            if label == 0 or label == 2:  # coordination phrase can end
                if cp != None and len(cp['conjuncts']) >= 2 and\
                        cp['cc'] > cp['conjuncts'][0][1] and\
                        cp['cc'] < cp['conjuncts'][-1][0]:
                    found = True
...
def dedup_extractions(extractions_list, conj_words):
    ...
    # Remove extractions which are almost equal
    # (threshold=0.9) - ignoring extractions which
    # are actually split (as they may be very similar)
    all_ext_words = []
    ...
    for i in range(len(all_ext_words)):
        for j in range(i+1, len(all_ext_words)):
            ...
            if difflib.SequenceMatcher(
                    None, ext_i_str, ext_j_str).ratio() > 0.9:
                if len_i > len_j:
...
class Carb:
    ...
    def __call__(self, predictions, sentences, scores, pos_words=None,
                ...
                if pro_extraction.args[0] != '' and pro_extraction.pred != '':
                    if self.mapping:
                        if not contains_extraction(
                                pro_extraction, self._all_predictions[
                        ...
                        if not contains_extraction(pro_extraction,
                                self._all_predictions[orig_sentence]):
                            self._all_predictions[orig_sentence].append(
                                pro_extraction)
        ...
        #             self.conj_word_mapping[sentence])
    ...
    def get_metric(self, reset, mode):
        if self.num_extractions:
            for sentence in self._all_predictions:
                self._all_predictions[sentence] = \
                    sorted(self._all_predictions[sentence],
                    key=lambda x: x.confidence,
                           reverse=True)[:self.num_extractions]

        out_filename = "/dev/null"
        if mode == 'dev':
            auc, optimal_f1_point, last_f1_point = \
                self._dev_benchmark.compare(predicted=self._all_predictions,
            ...
            auc, optimal_f1_point, last_f1_point = \
                self._test_benchmark.compare(predicted=self._all_predictions,
                                            matchingFunc=self.matchingFunc,
                                            output_fn=out_filename,
                                             error_file=None,
                                             binary=False)
