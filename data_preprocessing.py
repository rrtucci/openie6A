...
def label_arg2(extraction):
    def label_extraction(matches):
        if len(matches) == 2:
            assert matches[0].a == 0 and matches[0].size == matches[1].a and \
                   matches[1].size == 0
            extraction["arg2_tagged"] = True
            extraction["tags"][matches[0].b: matches[0].b +
                matches[0].size] = ["ARG2"] * matches[0].size

    if extraction['arg2'] == '' and len(
            extraction["args_tokens"]) == 0 and len(
            extraction["loc_args_tokens"]) == 0 and len(
            extraction["time_args_tokens"]) == 0:
        extraction['arg2_tagged'] = True

    elif seq_in_seq(extraction["arg2_tokens"]
        ...
        matches = difflib.SequenceMatcher(
            ...
            extraction["arg2_tokens"]
                  ...
                  + extraction["loc_args_tokens"]
                  + extraction["time_args_tokens"],
            extraction["tokens"]).get_matching_blocks()
        label_extraction(matches)

    elif seq_in_seq(
            extraction["arg2_tokens"] + extraction["args_tokens"]
            + extraction["time_args_tokens"]
            + extraction["loc_args_tokens"],
            extraction["tokens"]) == 1:
        matches = difflib.SequenceMatcher(None,
                    extraction["arg2_tokens"] +
                    extraction["args_tokens"] +
                    extraction["time_args_tokens"] +
                    extraction["loc_args_tokens"],
        ...
        label_extraction(matches)
            ...
            extraction["arg2_tokens"] + extraction["args_tokens"]
                + extraction["time_args_tokens"], extraction["tokens"]) == 1:
        matches = difflib.SequenceMatcher(None,
                extraction["arg2_tokens"] +
                extraction["args_tokens"] +
                extraction["time_args_tokens"],
        ...
        label_extraction(matches)
            ...
            extraction["arg2_tokens"] + extraction["args_tokens"] + extraction[
                "loc_args_tokens"], extraction["tokens"]) == 1:
        matches = difflib.SequenceMatcher(None, extraction["arg2_tokens"] +
                                          extraction["args_tokens"] +
                                          extraction["loc_args_tokens"],
                                          extraction[
                                              "tokens"]).get_matching_blocks()
        label_extraction(matches)

    elif seq_in_seq(
            extraction["arg2_tokens"] + extraction["time_args_tokens"] +
            extraction["loc_args_tokens"], extraction["tokens"]) == 1:
        matches = difflib.SequenceMatcher(None, extraction["arg2_tokens"] +
                                          extraction["time_args_tokens"] +
                                          extraction["loc_args_tokens"],
                                          extraction[
    ...
    elif seq_in_seq(extraction["arg2_tokens"] + extraction["loc_args_tokens"] +
                    extraction["time_args_tokens"], extraction["tokens"]) == 1:
        matches = difflib.SequenceMatcher(None, extraction["arg2_tokens"] +
                                          extraction["loc_args_tokens"] +
                                          extraction["time_args_tokens"],
    ...
    elif seq_in_seq(extraction["arg2_tokens"] + extraction["time_args_tokens"],
                    extraction["tokens"]) == 1:
        matches = difflib.SequenceMatcher(None, extraction["arg2_tokens"] +
                                          extraction["time_args_tokens"],
                                          extraction[
        ...
        label_extraction(matches)
            ...
            extraction["time_args_tokens"] + extraction["loc_args_tokens"],
            extraction["tokens"]) == 1:
        matches = difflib.SequenceMatcher(None,
                                          extraction["time_args_tokens"] +
                                          extraction["loc_args_tokens"],
        ...
        label_extraction(matches)
            ...
            extraction["loc_args_tokens"] + extraction["time_args_tokens"],
            extraction["tokens"]) == 1:
        matches = difflib.SequenceMatcher(None, extraction["loc_args_tokens"] +
                                          extraction["time_args_tokens"],
                                          extraction[
                                              "tokens"]).get_matching_blocks()
        label_extraction(matches)
        ...
        matches = difflib.SequenceMatcher(None, extraction["time_args_tokens"],
                                          extraction[
                                              "tokens"]).get_matching_blocks()
        label_extraction(matches)
...
def label_arg(extraction, arg):
    if seq_in_seq(extraction[arg + "_tokens"], extraction["tokens"]) == 1:
        ...
        extraction["tags"][matches[0].b: matches[0].b
            + matches[0].size] = [arg.upper()] * matches[0].size

    elif seq_in_seq(extraction[arg + "_tokens"], extraction["tokens"]) == 0:
        matches = difflib.SequenceMatcher(
            None, extraction[arg + "_tokens"],
            extraction["tokens"]).get_matching_blocks()
        if len(matches) > 2 and matches[0].a == 0 and all(
                matches[i].a == matches[i - 1].a
                + matches[i - 1].size for i in
                range(1, len(matches) - 1)) \
                and matches[-2].a + matches[-2].size == matches[-1].a:
            extraction[arg + "_tagged"] = True
            for match in matches:
                extraction["tags"][match.b: match.b + match.size] =\
                    [arg.upper()] * match.size
...
def label_is_of_relations(extractions):
    for extraction in extractions:
        if not extraction["rel_tagged"] and len(extraction["rel_tokens"]) > 0:
            if extraction["rel"] == '[is]':
            ...
            elif extraction["rel_tokens"][0] == '[is]' and \
                    extraction["rel_tokens"][-1] == '[of]':
                if len(extraction["rel_tokens"]) > 2 and seq_in_seq(
                        extraction["rel_tokens"][1:-1],
                        extraction["tokens"]) == 1:
                    matches = difflib.SequenceMatcher(
                        None, extraction["rel_tokens"][1:-1],
                        extraction["tokens"]).get_matching_blocks()
                    assert len(matches) == 2
                    assert matches[0].a == 0 and matches[0].size == matches[
                        1].a and matches[1].size == 0
                    extraction["rel_tagged"] = True
                    extraction["tags"][matches[0].b: matches[0].b +
                ...
                elif len(extraction["rel_tokens"]) > 2 and seq_in_seq(
                        extraction["rel_tokens"][1:-1],
                        extraction["tokens"]) == 0:
                    matches = difflib.SequenceMatcher(
                        None, extraction["rel_tokens"][1:-1],
                        extraction["tokens"]).get_matching_blocks()
                    if len(matches) > 2 and matches[0].a == 0 and all(
                            matches[i].a == matches[i - 1].a + matches[
                                i - 1].size for i in
                            range(1, len(matches) - 1)) and matches[-2].a + \
                            matches[-2].size == matches[-1].a:
                        extraction["rel_tagged"] = True
                        for match in matches:
            ...
            elif extraction["rel_tokens"][0] == '[is]' and len(
                    extraction["rel_tokens"]) > 1:
                assert not extraction["rel_tokens"][-1].startswith('[')
                if seq_in_seq(extraction["rel_tokens"][1:],
                              extraction["tokens"]) == 1:
                    matches = difflib.SequenceMatcher(
                        None, extraction["rel_tokens"][1:],
                        extraction["tokens"]).get_matching_blocks()
                    assert len(matches) == 2
                    assert matches[0].a == 0 and matches[0].size == matches[
                        1].a and matches[1].size == 0
...
def label_multiple_arg1(extractions):
    for extraction in extractions:
        if extraction['rel_tagged'] and not extraction[
            'arg1_tagged'] and seq_in_seq(extraction["arg1_tokens"],
                                          extraction["tokens"]) > 1:
            starting_indexes = [j for j in range(len(extraction["tokens"])) if
                                starts_with(
            ...
            if 'REL' in extraction['tags']:
                ...
                assert extraction['arg1_tokens'] == extraction['tokens'][
                    final_idx: final_idx + len(extraction['arg1_tokens'])]
                extraction['arg1_tagged'] = True
                extraction['tags'][
                final_idx: final_idx
                    + len(extraction['arg1_tokens'])] = ['ARG1'] * len(
                    extraction['arg1_tokens'])
            else:
                assert False
...
def label_multiple_rel(extractions):
    for extraction in extractions:
        ...
        if extraction['arg1_tagged'] and extraction['arg2_tagged'] and not \
        extraction['rel_tagged'] and len(extraction["rel_tokens"]) > 0:
            rel_tokens = None
            if seq_in_seq(extraction["rel_tokens"], extraction["tokens"]) > 1:
                rel_tokens = extraction["rel_tokens"]
            elif extraction["rel_tokens"][0] == '[is]' and seq_in_seq(
                    extraction["rel_tokens"][1:], extraction["tokens"]) > 1:
                rel_tokens = extraction["rel_tokens"][1:]
            elif extraction["rel_tokens"][0] == '[is]' and \
                    extraction["rel_tokens"][-1].startswith(
                            '[') and seq_in_seq(extraction["rel_tokens"][1:-1],
                                                extraction["tokens"]) > 1:
                rel_tokens = extraction["rel_tokens"][1:-1]

            if rel_tokens:
                starting_indexes = [j for j in range(len(extraction["tokens"]))
                                    if starts_with(
                        rel_tokens, extraction["tokens"], j)]
                assert len(starting_indexes) > 1

                min_dist = 10000000
                if 'ARG1' in extraction['tags'] and (
                        extraction['arg2'] == '' or 'ARG2' in extraction[
                    'tags']):
                    ...
                    if extraction['arg2'] == '':
                        ...
                        assert rel_tokens == extraction['tokens'][
                            final_idx: final_idx + len(rel_tokens)]
                        extraction['rel_tagged'] = True
...
def label_location(extractions):
    for extraction in extractions:
        if len(extraction["loc_args"]) == 1:
            ...
            ), extraction["tokens"]).get_matching_blocks() for arg in
                       extraction["loc_args"]]
            if all(len(match) == 2 for match in matches):
                if all(match[0].a == 0 and match[0].size == match[1].a and
                       match[1].size == 0 for match in matches):
                    for match in matches:
        ...
        if len(extraction["time_args"]) == 1:
            ...
            if all(len(match) == 2 for match in matches):
                if all(match[0].a == 0 and match[0].size == match[1].a and
                    ...
                    for match in matches:
                        extraction["tags"][
                        match[0].b: match[0].b + match[0].size] =\
                            ["TIME"] * match[0].size
...
def get_num_extractions(extractions):
    count = 0
    for extraction in extractions:
        if extraction['arg2_tagged'] and extraction['rel_tagged'] and \
                extraction['arg1_tagged']:
...
def process_merge_openie(lines, sentences, conj_mapping, selected_extractions):
    ...
    for line in lines:
        if len(line) > 0 and len(line.split('\n')) > 0:
            ...
            # and sentence not in wiki_sentences:
            if sentence in sentences or sentence in conj_mapping:
                # ignore extractions of original sentence - when
                # splits are available
                if conj_mapping != None and sentence in conj_mapping.values():
                    continue
                ...
                for i in range(1, len(extraction_text)):
                    ...
                    if extraction_num in selected_extractions \
                            or conj_mapping == None:
                        extraction = get_extraction(sentence,
                            parse_result_text(extraction_text[i]))
                        wiki_extractions.append(extraction)

                    # arg1_rel_tokens =
                    # f'{" ".join(extraction["arg1_tokens"])}
                    # {" ".join(extraction["rel_tokens"])}'
                    # ipdb.set_trace()
...
def process_allennlp(lines):
    ...
    for line in lines:
        ...
        for arg in ['arg1', 'rel', 'arg2']:
            ...
            extraction[arg] = ' '.join(
                re.findall(begin_tag + '.*' + end_tag, line[1])
                [0].strip(begin_tag).strip(end_tag).strip().split()
            )
        ...
        # extractionsD[extraction["text"]].add(f'{"
        # ".join(extraction["arg1_tokens"])} {"
        # ".join(extraction["rel_tokens"])}')

    return extractions, sentences, selected_extractions


def run_datpro(input_fp, output_fp, wiki_fp, conj_fp):
    ...
    with open(wiki_fp, 'r') as f:
        lines = f.read().split("\n\n")
    wiki_extractions = process_merge_openie(lines, oie4_sentences,
                                            conj_mapping, selected_extractions)
    ...
    with open(output_fp, 'w') as f:
        ...
        for extraction in wiki_extractions:
            if extraction['arg2_tagged'] and extraction['rel_tagged'] and \
                    extraction['arg1_tagged']:
                if 'REL' in extraction['tags'] and 'ARG1' in extraction[
                    'tags']:
                    if extraction['arg2'] == '' or 'ARG2' in extraction[
                        'tags']:
                        assert len(extraction['tokens']) == len(
                            extraction['tags'])
...
if __name__ == "__main__":
    ...
    # run_datpro(args.input_fp, args.output_fp, args.wiki_fp, args.conj_fp)

    def main():
        run_datpro(input_fp='oie4_extractions.tsv',
                   output_fp='openie5_seq5_new',
                   wiki_fp='wiki.txt.openie4.processed',
                   conj_fp='')
    main()
