...
light_verbs = ["take", "have", "give", "do", "make", "has", "have", "be", "is",
               "were", "are", "was", "had", "being", "began", "am",
               "following", "having", "do", "does", "did", "started", "been",
               "became", "left", "help", "helped", "get", "keep", "think",
               "got", "gets", "include", "suggest", "used", "see", "consider",
               "means", "try", "start", "included", "lets", "say", "continued",
               "go", "includes", "becomes", "begins", "keeps", "begin",
               "starts", "said", "stop", "begin", "start", "continue", "say"]
...
def make_datasets(hparams, predict_sentences=None):
    ...
    if 'predict' in hparams.mode:
        ...
        dev_dataset = [(len(ex.text), idx, ex, fields) for idx, ex in
                       enumerate(dev_examples)]
        test_dataset = [(len(ex.text), idx, ex, fields) for idx, ex in
...
def coords_to_sentences(conj_coords, words):
    ...
    word_sentences = [' '.join([words[i] for i in sorted(sentence)]) for
...
def remove_unbreakable_conjuncts(conj, words):
    ...
    unbreakable_words = ["between", "among", "sum", "total", "addition",
                         "amount", "value", "aggregate", "gross",
